...
def bid(id):
    ...
    run_auto_bid(id, session["user_id"])
...
def run_auto_bid(product_id, latest_user_id):
    ...
    next_price = current_price + get_bid_increment(current_price)

    if next_price > bot["max_price"]:
        db.close()
        return
    ...
    bot_username = bot_user["username"] if bot_user else "自動出價機器人"
...
@app.route("/auto-bid/<int:product_id>", methods=["POST"])
def auto_bid(product_id):
    ...
    if is_product_ended(product):
        db.close()
        create_trade_order_if_needed(product_id)
        flash("⛔ 此商品已結標，不能設定自動出價", "danger")
        return redirect(f"/product/{product_id}")
    ...
    if "is_paused" in product.keys() and product["is_paused"] == 1:
        db.close()
        flash("此商品目前已暫停販售，不能設定自動出價", "danger")
        return redirect(f"/product/{product_id}")

    if product["user_id"] == session["user_id"]:
        db.close()
        flash("賣家不能對自己的商品設定自動出價", "danger")
        return redirect(f"/product/{product_id}")

    min_auto_price = int(product["current_price"]) + get_bid_increment(product["current_price"])

    if max_price < min_auto_price:
        db.close()
        flash(f"自動出價上限至少要 NT$ {min_auto_price}", "danger")
        return redirect(f"/product/{product_id}")

    db.execute(
        """
        INSERT INTO auto_bids(product_id, user_id, max_price, created_at)
        VALUES (?, ?, ?, ?)
        """,
        (
            product_id,
